...
ax.plot(
    x, H, label="压头H/m", c=cy_c[1], marker=mks[2], markersize=12, lw=2, clip_on=False
)
ax.plot(
    x,
    He,
...
ax.legend(bbox_to_anchor=(0.28, 0.95), fontsize=16)  # loc表示图例位
ax.set_xlim(0, 15)
ax.set_ylim(5, 13)
...
ax1 = ax.twinx()
ax1.plot(
    x, eita, label="效率η/%", c=cy_c[3], markersize=12, marker=mks[5], lw=2, clip_on=False
)
ax1.set_ylim(0, 80)
ax1.legend(bbox_to_anchor=(0.25, 0.87), fontsize=16)  # loc表示图例位
...
ax2 = ax.twinx()
ax2.plot(
    x, P, label="功率P/kw", c=cy_c[4], markersize=12, marker=mks[6], lw=2, clip_on=False
)
ax2.set_ylim(0, 10)
ax2.legend(bbox_to_anchor=(0.25, 0.83), fontsize=16)  # loc表示图例位
