...
if add_selectbox=="遗传算法求解TSP问题":
   ...
   def drawpath(LJ, city_zb, num):
      ...
      ax.text(city_zb[LJ[0], 0] + 0.5, city_zb[LJ[0], 1] + 0.5, "起点")
      ax.text(city_zb[LJ[n - 1], 0] + 0.5, city_zb[LJ[n - 1], 1] + 0.5, "终点")
      for i in range(n):
   ...
   #print(gen, pre_obj)
   fig1, ax = plt.subplots(num="优化路径距离和遗传代数关系")
   while gen <= Maxgen:
   ...
   # pre_obj=p_len(index) 为上一轮最优
   #print(gen, pre_obj)
   fig=plt.figure(num="优化总价值和遗传代数关系")
   while gen <= Maxgen:
      GM = gene_mass(LJ, m)
      GV = gene_value(LJ, v)
      fitnv = fit(GM, GV, M)
   ...
   def drawpath(LJ, city_zb, num):
      ...
      ax.text(city_zb[0, 0] + 0.5, city_zb[0, 1] + 0.5, "邮局")
      ax.text(city_zb[LJ[0], 0] + 0.5, city_zb[LJ[0], 1] + 0.5, "起点")
      ax.text(city_zb[LJ[n - 1], 0] + 0.5, city_zb[LJ[n - 1], 1] + 0.5, "终点")
   ...
   ax.plot(x, y,color="b",label="目标函数值" ,linewidth=2.0, linestyle="--" )
   ...
   def PSO(fitness, N, c1, c2, w, M, D):
      ...
      fig,ax=plt.subplots(num="目标函数与迭代次数关系图")
      for i in range(M - 1):
   ...
   plt.text(xt, yt, "总长度=" + str(int(1000 * pen_best[id_best]) / 1000))
   st.write("绘制最终优化图")
   st.pyplot(draw_path)
   
   st.write("最优路径总长度=",(int(1000 * pen_best[id_best]) / 1000))
   fig,ax=plt.subplots(num="优化路径距离迭代次数关系图")
