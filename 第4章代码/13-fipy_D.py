...
u0 = fp.CellVariable(name = "u值求解结果", mesh = mesh, value = 0.)
u=u0
D = 10.
...
facesTopLeft = ((mesh.facesLeft & (Y >= L))
                | (mesh.facesTop & (X <= L)))
facesBottomRight = ((mesh.facesRight & (Y <= L ))
                    | (mesh.facesBottom & (X >= L)))
facesBottomLeft = ((mesh.facesLeft & (Y <= L ))
                    | (mesh.facesLeft & (X >=L)))
...
y=np.arange(nx)*dy
X,Y=np.meshgrid(x, y)
u0 = fp.CellVariable(name = "solution variable",
                   mesh = mesh, value = 0.)
for i in range(1,9):#i取值为1-8，不包含9
    ...
    p = ax[i//5,i-i//5*4-1].pcolor(X, Y, ar, cmap=mpl.cm.jet,norm=norm,shading='auto')#pcolor绘制
    ax[i//5,i-i//5*4-1].set_title("时间="+str(int(i*0.1*10)/10))
    ax[i//5,i-i//5*4-1].set_xticks([0,0.5,1.0,1.5,2])# 设置纵轴刻度
    ax[i//5,i-i//5*4-1].set_yticks([0,0.25,0.5,0.75,1.0,1.25,1.5,1.75,2])
