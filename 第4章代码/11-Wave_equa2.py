...
t0=30*np.ones((101,101))#设置初始解为零
#t0[100,:]=100.0#设置右边界条件
t0[:,100]=30.0#设置上边界条件
#t0[0,:]=100.0#设置左边界条件
#设置下部边界为绝热t1_(:,-1)=t_1(:,1)
print(t0)
t1=30*np.ones((101,101))#设置初始解为零
for i in range(101):
      ...
      t0[0,i]=100.0#设置左边界条件
      t0[100,i]=100.0#设置右边界条件
...
x=np.arange(101)*0.01#长度位置，归一处理，从0开始，共1001个点
...
while flag==True:
    ...
    for i in range(1,100):#内部点迭代
        for j in range(1,100):
            t1[i,j]=0.25*(t0[i-1,j]+t0[i,j-1]+t0[i+1,j]+t0[i,j+1])#迭代计算
        t1[i,0]=0.25*(t0[i-1,0]+2*t0[i,1]+t0[i+1,0])


    if np.max(abs((t1-t0)/t0))<=0.00001:
        flag=False
    #print(k)
    for i in range(1,100):#内部点迭代替换,含第一排数据
...
p = ax.pcolor(X, Y, t0.T, cmap=mpl.cm.jet,norm=norm,shading='auto')#pcolor绘制
