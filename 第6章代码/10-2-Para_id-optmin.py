...
# 输入数据
C1 = np.array(
    [10, 8.6231, 7.4627, 6.4811, 5.6475, 4.9366, 4.3282, 3.8057, 3.3552, 2.9652, 2.6261]
)
C2 = np.array(
    [0, 1.2759, 2.1736, 2.7808, 3.1666, 3.385, 3.478, 3.4783, 3.4118, 3.2984, 3.1537]
)
C3 = np.array(
    [0, 0.202, 0.7273, 1.4761, 2.3719, 3.3569, 4.3876, 5.432, 6.4661, 7.4728, 8.4403]
)
...
def fun(x):
    ...
    # sum=J(k1,k2,k3)
    JJ = sum(
        sum((odeint(dy, y0, tspan, args=(k1, k2, k3)) - C) ** 2)
...
res = op.minimize(
    fun, k0, method="L-BFGS-B", bounds=[(0.01, 10), (0.01, 10), (0.01, 10)]
)
k = res.x  # 获取参数
