# Temperature-presure  fit  for test
# 15-test-tem_pre-fit.py
from re import A
...
xdata = np.array([-10, 0, 20, 40, 60, 80, 100])
y_real = np.array([221, 307, 558, 944, 1505, 2286, 3328])
# 读入拟合数据
...
for i in range(55):
    ...
    y_real = np.array([221, 307, 558, 944, 1505, 2286, 3328]) + i * y0
    alf0 = [12, -100, 30]  # 初值对拟合结果有较大影响
...
if alf_opt[2] < 0:
    if alf_opt[1] < 0:
        plt.text(
            min(xdata),
            0.6 * max(y_real),
            f"拟合方程：lnP={alf_opt[0]:.5f}{alf_opt[1]:.5f}/(T{alf_opt[2]:.5f})",
        )
    else:
        plt.text(
            min(xdata),
