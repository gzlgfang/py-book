...
b_ub = [38, 30, 36, 28, 48]
print(A_ub)
bounds = ((0, None), (0, None), (2, None))
for i in range(10):
    c1 = -12 - i
    c = [c1, -12, -8]
    r = linprog(c, A_ub=A_ub, b_ub=b_ub, bounds=bounds, method="simplex")
    # print(r)
    J[i] = r.fun
    x[i, :] = r.x
    # print(x[i,:])
...
plt.plot(
    x_c1, y_j, lw=2, color="b", label="0.1J", marker="o", markersize=12
)  # 绘制c1和J之间的函数曲线
plt.plot(
    x_c1, x[:, 0], lw=2, color="r", label="x1", marker="*", markersize=12
)  # 绘制c1和x1之间的函数曲线
plt.plot(
    x_c1, x[:, 1], lw=2, color="k", label="x2", marker="d", markersize=12
)  # 绘制c1和x2之间的函数曲线
plt.plot(
    x_c1, x[:, 2], lw=2, color="purple", label="x3", marker="p", markersize=12
)  # 绘制c1和x3之间的函数曲线
...
y_j = -0.1 * J  # 目标函数乘0.1以便和x的值比例相似
plt.plot(
    x_b2, y_j, lw=2, color="b", label="0.1J", marker="o", markersize=12
)  # 绘制b2和J之间的函数曲线
plt.plot(
    x_b2, x[:, 0], lw=2, color="r", label="x1", marker="*", markersize=12
)  # 绘制b2和x1之间的函数曲线
plt.plot(
    x_b2, x[:, 1], lw=2, color="k", label="x2", marker="d", markersize=12
)  # 绘制b2和x2之间的函数曲线
plt.plot(
    x_b2, x[:, 2], lw=2, color="purple", label="x3", marker="p", markersize=12
)  # 绘制b2和x3之间的函数曲线
...
y_j = -0.1 * J  # 目标函数乘0.1以便和x的值比例相似
plt.plot(
    x_b1, y_j, lw=2, color="b", label="0.1J", marker="o", markersize=12
)  # 绘制b1和J之间的函数曲线
plt.plot(
    x_b1, x[:, 0], lw=2, color="r", label="x1", marker="*", markersize=12
)  # 绘制b1和x1之间的函数曲线
plt.plot(
    x_b1, x[:, 1], lw=2, color="k", label="x2", marker="d", markersize=12
)  # 绘制b1和x2之间的函数曲线
plt.plot(
    x_b1, x[:, 2], lw=2, color="purple", label="x3", marker="p", markersize=12
)  # 绘制b1和x3之间的函数曲线
