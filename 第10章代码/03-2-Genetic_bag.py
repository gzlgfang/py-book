...
#v=m_v[1,:]
#输入确定的物体重量及价值
v=np.array([202,208,198,192,180,180,165,162,160,158,155,130,125,122,120,118,115,110,105,101,110,100,98,96,95,90,88,82,80,77,75,73,72,70,69,66,65,63,60,58,56,50,30,20,15,10,8,5,3,1])
m=np.array([80,82,85,70,72,70,66,50,55,25,50,55,40,48,50,32,22,60,30,32,40,38,35,32,25,28,30,22,50,30,45,30,60,50,20,65,20,25,30,10,20,25,15,10,10,10,4,4,2,1])
n=len(m)
M=1000 #背包中可以放入的总重量
...
def draw_T_x(T_x,GM,GV,index,num):
    ...
    x=np.arange(1,n+1)
    y=T_x[index]
    plt.scatter(x,y,s=100,c='red',cmap=mpl.cm.RdYlBu,clip_on=False)#clip_on=False表示坐标轴上的Mark也要整体显示
    plt.plot(x,y,lw=2)
