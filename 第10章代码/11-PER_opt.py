...
def PSO(fitness, N, c1, c2, w, M, D):
    ...
    for i, val in enumerate(x):
        p[i] = fitness(val, D)  # 计算适应度，即目标函数，计算N个粒子的函数值
    # gbest 全局最优位置
    gbest = x[N - 1]
        ...
        for i in range(N):
            ...
            for j in range(D):
                ...
                if x[i, j] > 1:  # 保证变量不大于1，需要根据具体求解问题设置
                    x[i, j] = 1
            if fitness(x[i], D) < p[i]:  # 更新个体极值
                p[i] = fitness(x[i], D)
                pbest[i] = x[i]  # pbest[i]为个体最优解
            if p[i] < fitness(gbest, D):  # 更新全局极值
                gbest = pbest[i]
        pbest_fit[t] = fitness(gbest, D)
        x.sort
    # 结束迭代，得到最优解gbest,
    ...
    tspan = np.linspace(0, 1, D)  # 确定自变量范围
    C_tspan = np.zeros((2, tspan.size))  # 确定2个应变量初值

    def dx(t, xx):  # t表示长度方向，xx表示对应物质摩尔分率
        ipo = spi.splrep(tspan, gbest, k=3)  ##将gbest以tspan作为自变量进行拟合
        ky = spi.splev(t, ipo)  # 将自变量代入拟合的ipo中，确定微分方程中需要的ky,这是
        x1, x2 = xx[0], xx[1]  # 一个变参数微分方程组求解
    ...
    x = np.linspace(0, 1, D)
    plt.plot(x, T, color="b", linewidth=3)
    plt.grid()
    plt.xticks(np.linspace(0, 1, 11, endpoint=True))  # 设置横轴刻度
    plt.xlim(0, 1)
    plt.xlabel("管长，L")
