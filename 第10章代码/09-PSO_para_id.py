...
#定义边界条件
    ...
    for i in range(len(CA_E)):
       fit=fit+ (CA_C[i]-CA_E[i])**2+(CB_C[i]-CB_E[i])**2 +(CC_C[i]-CC_E[i])**2+(CD_C[i]-CD_E[i])**2    
    return fit

#已知数据
CA_E=np.array([10,9.048,8.187,7.408,6.703,6.065,5.488,4.965,4.493,4.065,3.678])
CB_E=np.array([0,0.882,1.559,2.076,2.497,2.892,3.298,3.708,4.104,4.475,4.816])
CC_E=np.array([0,0.138,0.506,1.03,1.598,2.084,2.426,2.65,2.802,2.916,3.008])
CD_E=np.array([0,0.069,0.253,0.515,0.799,1.042,1.213,1.325,1.401,1.458,1.504])

PSO(fitness, 50, 1.5, 2.5, 0.5, 50, 3)#调用总程序
plt.show()
