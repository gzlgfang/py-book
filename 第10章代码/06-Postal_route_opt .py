...
# 读入城市坐标
DF = pd.read_excel(
    "g:/Postal.xlsx", "Sheet1", na_filter=False, index_col=0
)  # 共有21个位置坐标, 第0个为邮局
city_x = np.array(DF["x"])  # 数据分配
city_y = np.array(DF["y"])
w1 = np.array(DF["w"])
n = len(city_x) - 1  # 计算投递点的数目
w = w1[1:]  # 读入每个投递点的投递重量，共n个
TG = sum(w)
print("总邮件重量=", TG)
...
def drawpath(LJ, city_zb, num):
    ...
    for i in range(n):  # 0-n-1
        ...
        plt.text(
            city_zb[i + 1, 0] - 0.3, city_zb[i + 1, 1] + 0.5, str(i + 1), color="r"
        )
    # 绘线
...
# 判断新路径是否被采用
def Metropolis(LJ1, LJ2, D, T):
    Len1 = pathlength(D, LJ1)  # 计算轨迹LJ1路径长度
...
for test in range(test_num):
    ...
    while T0 > Tend:
        ...
        for i in range(L):
            ...
            LJ2 = Newpath(LJ0)
            # Metropolis 法则判断新解
            [LJ0, Len] = Metropolis(LJ1, LJ2, D, T0)
    ...
    opt_JJ[test] = p_len[0]
    opt_way[test, :] = LJ0
    plt.figure(num="优化路径目标函数和退火次数关系图")
