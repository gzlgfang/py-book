...
def drawpath(LJ,city_zb,num):
    ...
    plt.annotate('',xy=xy,xytext=xytext , arrowprops=dict(arrowstyle='<-',color="g",lw=2))
...
while itera_num<itera_max:
    ...
    for i in range(m):  #m只蚂蚁逐个城市选择路径
        for j in range(1,n):
            ...
            for k in range(len(allow)):
                   ...
                   P[k]=tau[prohi_tab[j-1],allow[k]]**alpha+eta[prohi_tab[j-1],allow[k]]**beta
    ...
    for i in range(m):
        for j in range(n-1):
            detal_tau[LJ[i,j],LJ[i,j+1]]=detal_tau[LJ[i,j],LJ[i,j+1]]+Q/ p_len[i]
            #detal_tau[LJ[i,j+1],LJ[i,j]]=detal_tau[LJ[i,j+1],LJ[i,j]]+Q/ p_len[i]
        detal_tau[LJ[i,n-1],LJ[i,0]]=detal_tau[LJ[i,n-1],LJ[i,0]]+Q/ p_len[i]#最后一个点和起始点闭合
        #detal_tau[LJ[i,0],LJ[i,n-1]]=detal_tau[LJ[i,0],LJ[i,n-1]]+Q/ p_len[i]
    
    
    #最优蚂蚁路线加强：
    for j in range(n-1):
            detal_tau[LJ[id_ant,j],LJ[id_ant,j+1]]=detal_tau[LJ[id_ant,j],LJ[id_ant,j+1]]+9
            #detal_tau[LJ[i,j+1],LJ[i,j]]=detal_tau[LJ[i,j+1],LJ[i,j]]+Q/ p_len[i]
    detal_tau[LJ[id_ant,n-1],LJ[id_ant,0]]=detal_tau[LJ[id_ant,n-1],LJ[id_ant,0]]+9
